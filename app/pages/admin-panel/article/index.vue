    ...
    <v-body>
        <div>
           <v-card outlined>
                ...
                >
                    <div class="pt-6">
                        <v-select :disabled="!isEdit" solo dense :items="listItems" v-model="item.sort" @change="updateSort(item)"></v-select>
                    </div>
                </template>
                <template
...
export default {
    ...
    methods:{
        async getTatal() {
            const { data } = await axios.get('/api/article/count');
            try {
        ...
        async updateSort(item) {
            try {
                const { data } = await axios.patch(`/api/article/${item._id}/${item.sort}`);
        ...
        async deleteItem(item) {
            try {
                const { data } = await axios.delete(`/api/article/${item._id}`);
                await this.$refs.apiTable.getData(true);
                try {
    ...
    async mounted(){
        ...
        this.$checkPermission("article","create").then(r => {
        this.isCreate = r;
        });
        this.$checkPermission("article","edit").then(r => {
        this.isEdit = r;
        });
        this.$checkPermission("article","remove").then(r => {
        this.isDelete = r;
        });
    }
};
