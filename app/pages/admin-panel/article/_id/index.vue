...
export default {
  components: { VTuiEditor },
    ...
    methods:{
        async syncData(){
            const $r = await axios.get(`/api/article/${this.$route.params.id}`);
        ...
        async save(){
            ...
            const $r = await axios.post(`/api/article/${this.$route.params.id}`, this.field);
            this.$router.push(`/admin-panel/article`);
        },
    },
    async mounted(){
        ...
        this.isEdit=  this.$checkPermission("article","edit");
